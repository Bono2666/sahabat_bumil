...
class _UpdProfileState extends State<UpdProfile> {
  ...
  Future updProfile() async {
    var url = Uri.parse('https://sahabataqiqah.co.id/sahabat_bumil/api/upd_profile.php?phone=${prefs.getPhone}&name=${_name.text}&email=${_email.text}');
    var response = await http.get(url);
    return json.decode(response.body);
  }

  Future updVerified() async {
    var url = Uri.parse('https://sahabataqiqah.co.id/sahabat_bumil/api/upd_verified.php?phone=${prefs.getPhone}&verified=$_verified');
    var response = await http.get(url);
    return json.decode(response.body);
  }
                                ...
                                if (_email.text != '') {
                                  if (EmailValidator.validate(_email.text)) {
                                    ...
                                    if (auth.currentUser.uid != null) {
                                      ...
                                      await Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => UpdProfile(),));
                                    }
                                  } else {
                                    setState(() {
                                      isInvalid = true;
